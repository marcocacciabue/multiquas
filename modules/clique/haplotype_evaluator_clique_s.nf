...
process HAPLOTYPE_EVALUATOR_CLIQUE_S {
   label 'process_low'
  tag "$sample_id"
  container "cacciabue/multiquas:developing"
  publishDir "results/${sample_id}/haplotypes_${reconstructer}/", mode: 'copy'
  ...
  output:
     ...
     path("${haplotypes}"),
     path("${proportions}"),
     path("${sample_id}_${reconstructer}_graphs.png"),
     path("${sample_id}_R_sq.txt"),emit:reconstructed_data
     path("${sample_id}_single_${reconstructer}_results.txt"),emit:results
...
query_alineado <- read.fasta('${haplotypes}',
                             seqtype = "DNA")
...
ggsave('${sample_id}_${reconstructer}_graphs.png',
       p,
       units="in", 
       width=10, 
       height=8,
...
'${sample_id}_single_${reconstructer}_results.txt', 
sep=",",
row.names=FALSE)
